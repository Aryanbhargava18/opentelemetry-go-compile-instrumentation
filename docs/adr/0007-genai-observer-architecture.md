...
*   **State Aggregation:** Because tokens and tool calls are streamed incrementally, the `genai.Observer` tracks running maximums for tokens (preventing double-counting if a provider sends cumulative usage per-chunk), concatenates streamed `ToolCall.Args` by ID, and persists single-event fields (like `Model` and `ID`) across the stream lifecycle. The `ChunkExtractor` remains strictly stateless.
